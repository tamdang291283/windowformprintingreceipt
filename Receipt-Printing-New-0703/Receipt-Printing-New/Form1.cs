...
namespace Receipt_Printing_New
{
    ...
    public partial class Form1 : Form
    {
        ...
        public string PrinterSettingLog = "";
        ...
        public Form1( string noloaddata)
        {
            InitializeComponent();
        }
        private string GetRestaurantid()
        {
            string restaurantid = "";
            string urlordercome = "";
            string PrintSettingPath = System.IO.Path.GetFullPath(@"PrinterSetting\");
            ...
            if (File.Exists(printsettingfile))
            {
                using (StreamReader r = new StreamReader(printsettingfile))
                {
                    string line;
        ...
        public void LoadTabSetting()
        {
            try
            {
                ...
                if (!exists)
                    System.IO.Directory.CreateDirectory(PrintSettingPath);
                printsettingfile = PrintSettingPath + "setting.txt";
                PrinterSettingLog = PrintSettingPath + "PrinterSettingLog.txt";
                // Detect delete file
               // DeleteLogFile2week();
                int index = 0;
        ...
        private void printing(string url, string imgfilename, string papersize)
        {
            try
            {
                ...
                int paperwidth = paperwidth = papersize.Contains("80") ? 295 : 213;
            ...
            try
            {
                if (printed == false)
                {
                    if (txtAddressURL.Text.ToString().Trim() != "")
                    {
                        ...
                        int numberModule = 0;
                        int rowNumber = 0;
                       List<SettingExtend> lst = new List<SettingExtend>();
                        // myPrinters.SetDefaultPrinter(cbPrinterList.Text.ToString().Trim());

                        if (File.Exists(System.IO.Path.GetFullPath("PrinterSetting\\PrinterSettingLog.txt")))
                        {
                            using (StreamReader readtext = new StreamReader("PrinterSetting\\PrinterSettingLog.txt"))
                            {
                                string printername = "";
                                string printersize = "";
                                ...
                                while (true)
                                {
                                    ...
                                    //SettingExtend SE = new SettingExtend();
                                    //SE.parameters = data[0];
                                    //SE.isfullreceipt= bool.Parse( data[1] =="" ? "false": data[1]);
                                    //SE.listCategories = data[2];
                                    //SE.printers = data[3];
                                    //SE.copies = data[4];
                                    //SE.sizes= data[5];
                                    //lst.Add( SE );
                                    ...
                                    {
                                        printername = data[3];
                                        myPrinters.SetDefaultPrinter(printername.Trim());
                                    }                                        
                                    else if (printername != data[3])
                                    {
                                        printername = data[3];
                                        myPrinters.SetDefaultPrinter(printername.Trim());
                                    }
                                    ...
                                    numberModule++;
                                    int numbercopies = string.IsNullOrEmpty(data[4]) ? 0:  int.Parse(data[4]);

                                    string responseText = getResponsePostRequest(txtAddressURL.Text.ToString().Trim() + "&settinglist=" + readText);
                                                ...
                                                if (url != "")
                                                {
                                                    try
                                                    {
                                                        if (url.Contains("dishname"))
                                                        {
                                                            ...
                                                            printing(url, ResID + "-" + OrderID + "-EN" + countEN, printersize);
                                                        ...
                                                        else
                                                        {
                                                            if (EnableSound.Checked == true)
                                                                playPrinting();
                                                            WriteLog(url, "printing: ");
                                                            printing(url, ResID + "-" + OrderID + "-CN" + countCN, printersize);
                                                            WriteLog("Success!", "printing: ");
                                                            countCN += 1;
                                                        }
                                                    }
                        ...
                        //foreach(SettingExtend SE in lst)
                        //{
                        //    bool isonline = false;
                        //    bool islocal = false;
                        //    bool isprintingname = false;
                        //    bool issplititems = false;
                        //    string URL = "";

                        //   if(SE.parameters.Contains("Online") )                       
                        //        isonline = true;
                        //    if (SE.parameters.Contains("Local"))
                        //        islocal = true;
                        //    if (SE.parameters.Contains("Use Printing"))
                        //        isprintingname = true;
                        //    if (SE.parameters.Contains("Split Items"))
                        //        issplititems = true;
                        //    if(isonline == true || islocal==true)
        ...
        private void LoadPrinterSetting()
        {
            ...
            int numberModule = 0;
            int rowNumber = 0;
            if (File.Exists(PrinterSettingLog))
            {
                using (StreamReader readtext = new StreamReader(PrinterSettingLog))
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            if(isfullreceipt.ToLower() !="true")
                            {
                                var cklstboxfullreceipt = ((CheckedListBox)module.Controls.Find("cklstboxfullreceipt", true)[0]);
                                string sreceiptclone = sreceipt.Trim();
                                if (sreceiptclone != "")
                                    sreceiptclone = sreceiptclone + ",";
                                for (int i = 0; i < cklstboxfullreceipt.Items.Count; i++)
                                {
                                    ...
                                    foreach (string s in lst)
                                    {
                                        if (itemCategory.svalue == s)
                                        {
                                            ...
                                            sreceiptclone = sreceiptclone.Replace(s + ",", "");
                                ...
                                if (sreceiptclone!="")
                                {
                                    sreceiptclone = sreceiptclone.TrimEnd(',');
                                    messagebox = "category " + sreceiptclone + " is changed on store. Please select another category.";
        ...
        private void btAddNewMapping_Click(object sender, EventArgs e)
        {
            ...
            ucModule1 module = new ucModule1(restaurantid);
            module.setMapping(numberModule + 1);
        ...
        private void btsave_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(PrinterSettingLog))
                File.Delete(PrinterSettingLog);
            if (lstMappingData.Count > 0)
            {
                using (StreamWriter writetext = new StreamWriter(PrinterSettingLog))
                {
    ...
    public class SettingExtend
    {
        public string parameters { get; set; }
        public bool isfullreceipt { get; set; }
